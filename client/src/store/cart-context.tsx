...
export const CartContextProvider = (props: CartContextProps) => {
  ...
  const onChangeHandler = (selectMeal: SelectedMeals) => {
    const filteredCartItems = cartItems.filter((cartItem) =>
      cartItem.id === selectMeal.id ? cartItem : null
    );
    if (filteredCartItems.length) {
      const existingSelectedItem = cartItems
        .map((cartItem) => {
          if (cartItem.id === selectMeal.id) {
            const improveSelectedMealAmount = {
              ...cartItem,
              amount: selectMeal.amount + cartItem.amount,
            };
            return improveSelectedMealAmount.amount
              ? improveSelectedMealAmount
              : null;
          }
          return cartItem;
        })
      ...
      localStorage.setItem(
        "cart",
        JSON.stringify(existingSelectedItem.length ? existingSelectedItem : [])
      );
  ...
  return (
    <CartContext.Provider
      value={{ onChangeHandler: onChangeHandler, cartItems: cartItems }}
    >
      {" "}
      {props.children}{" "}
    </CartContext.Provider>
